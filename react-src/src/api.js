...
export function getDaySchedules( tourId, date, lang = 'es' ) {
  return request( `availability/day?tour_id=${tourId}&date=${date}&lang=${lang}` );
}

// ── Precios ───────────────────────────────────────────────────────────────────

export function getQuote( { tourId, scheduleId, date, adults, children, babies } ) {
  return request( 'bookings/quote', {
    method: 'POST',
    body: JSON.stringify( { tour_id: tourId, schedule_id: scheduleId, date, adults, children, babies } ),
  } );
}
