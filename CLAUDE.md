...
- **Plugin en v2.1.0** (`AMIR_VERSION`), `AMIR_DB_VERSION` en 1.8.8. Último ZIP armado: `amir-booking-2.1.0.zip` (no queda guardado entre sesiones — el scratchpad es efímero; se rearma con `composer install --no-dev --optimize-autoloader` + `zip`, ver regla 2. Si hay cambios en `react-src/`, correr `npm run build` ahí primero).
- **Todo el trabajo de esta rama ya está commiteado y pusheado a `origin/fase1/seguridad-base-codigo2`** (dos commits grandes: uno de trabajo acumulado de sesiones previas nunca commiteado — multi-idioma real, tema visual del widget, calendario admin, rename Mercado Pago — y otro con todo lo de esta sesión). Si en una sesión nueva `git status` muestra cambios sin commitear al arrancar, probablemente sea trabajo de ESA sesión sin cerrar, no algo perdido.
- **El sandbox venía en v1.6.0** (varias sesiones de trabajo acumuladas sin subir) — se subió hasta v1.9.2 en esta sesión para diagnosticar el bug de la lista de interés (ver abajo). **v2.1.0 todavía no se subió al sandbox ni se probó en vivo** — es lo primero para la próxima sesión: subir el ZIP y volver a probar el tour "Yoga en el Cielo" (post 8609) para confirmar que la lista de interés sincroniza, y probar el resto de lo nuevo (Pixel/Ads con credenciales de prueba, `?coupon=`, tour solo-adultos).
