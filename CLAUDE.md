...
- **Plugin en v2.1.2** (`AMIR_VERSION`), `AMIR_DB_VERSION` en 1.8.8 (sin cambios de esquema esta sesión). Último ZIP armado: `amir-booking-2.1.2.zip` (no queda guardado entre sesiones — se rearma con `composer install --no-dev --optimize-autoloader` + `zip`, ver regla 2). **Ojo con el tamaño del ZIP**: `vendor/` pesa ~43MB (`tecnickcom/tcpdf` + `endroid/qr-code`, dependencias reales) — excluir también `dev-notes/` y `package-lock.json` además de lo obvio.
...
- **v2.1.2 (esta sesión) todavía NO se subió al sandbox** — es lo primero para la próxima: subir el ZIP y:
