...
- **Plugin en v2.1.3** (`AMIR_VERSION`), `AMIR_DB_VERSION` en 1.8.8 (sin cambios de esquema esta sesión). Último ZIP armado: `amir-booking-2.1.3.zip` (no queda guardado entre sesiones — se rearma con `composer install --no-dev --optimize-autoloader` + `zip`, ver regla 2). **Ojo con el tamaño del ZIP**: `vendor/` pesa ~43MB (`tecnickcom/tcpdf` + `endroid/qr-code`, dependencias reales) — excluir también `dev-notes/` y `package-lock.json` además de lo obvio.
- **Todo el trabajo de esta rama ya está commiteado y pusheado a `origin/fase1/seguridad-base-codigo2`.**
- **v2.1.0/2.1.1 SÍ se subieron al sandbox y se probaron en vivo** — confirmado que el bug de `content_i18n` quedó resuelto (tour "Yoga en el Cielo" sincroniza bien, wishlist genera reserva real y pasa a `awaiting_payment`), y que el tour solo-adultos (`allow_children`/`allow_babies`) oculta los contadores correctamente en el widget.
- **v2.1.2 y v2.1.3 (esta sesión) todavía NO se subieron al sandbox** — es lo primero para la próxima: subir el ZIP y:
  1. **Borrar/cancelar la reserva de prueba `id=48`** ("Test Admin Booking", `johelpastorino+admintest@gmail.com`, Kayak al Amanecer 2026-08-10) — quedó creada en el sandbox al probar el formulario de reserva manual, invisible hasta que este ZIP suba (ver bug de JOIN abajo).
  2. Confirmar con la página **"✉️ Probar emails"** (TourFlow → Probar emails) si el fix del asunto de `TourOpenedEmail` (sacar emoji + "completá tu pago") resolvió la entrega — no se pudo confirmar en esta sesión porque esa página es parte de este mismo ZIP.
  3. **Probar "📱 Modo campo" desde el celular real** (ver más abajo) — se construyó y se revisó el código, pero no se pudo probar en vivo porque todavía no está en el sandbox.
- **Nueva pantalla "📱 Modo campo"** (`includes/admin/class-field-page.php`, menú TourFlow → Modo campo) — pensada para gestionar reservas desde el celular en la calle/el muelle, no un retrofit responsive de Reservas (esa tabla es demasiado densa para un teléfono). Fullscreen sin el chrome de WP (mismo mecanismo que ya usaba Calendario, generalizado: `AdminMenu::maybe_hide_admin_bar_fullscreen()`/`maybe_print_fullscreen_css()` ahora aceptan una lista de páginas fullscreen, no una sola hardcodeada). Cubre lo que se priorizó en el diálogo con el cliente: buscar/ver reservas (hoy/próximas/todas), ver datos del cliente y contactarlo (`tel:`/`wa.me` directo desde la tarjeta), confirmar, reprogramar, cancelar, y cargar una reserva nueva — todo reusando `BookingManager` tal cual (mismos métodos que ya usa Reservas de escritorio, cero lógica de negocio nueva, solo presentación). Portal de autogestión para proveedores externos (marketplace, ver § 11 de CONTRIBUTING.md) es candidato a reusar este mismo patrón fullscreen el día que se construya.
- **Bug real encontrado y corregido — JOIN en vez de LEFT JOIN con `amir_tour_schedules`** (4 lugares: `class-dashboard-page.php`, `class-reports-page.php`, `class-bookings-page.php` ×2 — lista y detalle). Cualquier reserva con `schedule_id=0` ("sin horario específico", una opción válida tanto en tours de un solo horario como en el formulario de reserva manual) quedaba **invisible** en Reservas, Reportes y el resumen del día del Dashboard/Calendario, aunque la fila existiera en la base — se encontró probando el formulario "Nueva reserva manual" con `awaiting_payment` (ver debajo), que devolvía "Reserva no encontrada" pese a haber creado la reserva con éxito (el conteo total subió de 38 a 39). Los otros 5 archivos que hacen el mismo JOIN (`class-cron-manager.php`, `class-shortcodes.php`, `class-voucher-generator.php`, `class-email-dispatcher.php`, `class-wishlist-page.php`) ya usaban `LEFT JOIN` correctamente — sirvió de referencia para el fix.
- **Documentación desactualizada, corregida**: dos ítems marcados "⏳ Parcial/Pendiente" en el roadmap ya estaban implementados end-to-end de sesiones previas sin actualizar el doc — **add-ons en el checkout** (`StepExtras` en `BookingWidget.jsx`, ya conectado a `ToursController::fetch_addons()`) y **cargar reserva + link de pago desde el admin** (`BookingsPage::render_new_booking_form()`, checkbox "El cliente todavía no pagó" → `create_manual()` con `awaiting_payment` → `send_payment_link_notice()`). Antes de agregar algo "que falta" según CLAUDE.md/CONTRIBUTING.md, vale la pena grep-ear el código primero — la doc se atrasa más rápido de lo que parece.
- **Cupones**: ahora se pueden editar (antes solo crear/pausar/borrar) — botón "Editar" por fila en Amir Booking → Cupones, no permite bajar el límite de usos por debajo de lo ya usado.
- **Checkbox de política de cancelación**: ahora se valida server-side (`BookingManager::create_pending()` rechaza la reserva si `policy_accepted` no llega en `true` — antes solo el frontend deshabilitaba el botón, la API se podía llamar directo sin aceptar nada) y el texto es configurable (Configuración → política de cancelación, ES/EN — vacío = siguen las 3 líneas por defecto de siempre).
- **Higiene de nombres**: `amir_process_stripe_refund` → `amir_process_gateway_refund`; `BookingManager::confirm()` ahora escribe tanto `stripe_charge_id` (compat) como `gateway_charge_id` (genérico), y las lecturas prefieren el genérico con fallback al viejo.
- **Purchase a píxeles usa el monto real**: `StepConfirm` reportaba `quote.total_mxn` (el último cálculo del cliente) — ahora usa el `total_mxn` que devolvió el backend al crear la reserva (`result.total_mxn`), evitando reportar un valor desactualizado si el precio cambió justo al confirmar.
- Patrón que siguió sirviendo esta sesión: para descartar un bug sin desplegar nada, comparar contra otra pantalla admin que use el mismo mecanismo (SQL o email) — así se confirmaron sin ambigüedad el fix de `content_i18n`, que el email de wishlist usa el mismo `wp_mail()`/GoSMTP que el resto, y el bug de JOIN de arriba (probando en vivo con Browser, no asumiendo).
