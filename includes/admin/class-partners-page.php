...
class PartnersPage {
    ...
    private function render_list( string $message = '' ): void {
        ...
        <h1 style="display:flex;align-items:center;justify-content:space-between;">
          <span>🤝 <?php echo esc_html( $this->tt( 'Partners', 'Partners' ) ); ?></span>
          <button onclick="document.getElementById('amir-new-partner-form').style.display='block';this.style.display='none';"
                  class="button button-primary">+ <?php echo esc_html( $this->tt( 'Nuevo partner', 'New partner' ) ); ?></button>
        </h1>
        ...
        <div id="amir-new-partner-form" style="display:none;background:#fff;border:1px solid #e1f5ee;border-radius:10px;padding:20px;margin-bottom:24px;">
          ...
          <form method="post">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:14px;">
              <div><?php $this->field( $this->tt('Nombre / Empresa','Name / Business'), 'name', 'text', 'Hotel Laguna…', true ); ?></div>
              <div><?php $this->field( 'Email', 'email', 'email', 'contacto@hotel.com', true ); ?></div>
              <div><?php $this->field( $this->tt('Teléfono','Phone'), 'phone', 'tel', '+52 983…' ); ?></div>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:14px;">
              <div>
                <label class="ab-label"><?php echo esc_html( $this->tt( 'Tipo de comisión', 'Commission type' ) ); ?></label>
                <select name="commission_type" style="<?php echo $this->input_style(); ?> width:100%;">
                  <option value="percentage"><?php echo esc_html( $this->tt( 'Porcentaje (%)', 'Percentage (%)' ) ); ?></option>
                  <option value="fixed"><?php echo esc_html( sprintf( $this->tt( 'Monto fijo (%s)', 'Fixed amount (%s)' ), \AmirBooking\Core\Currency::code() ) ); ?></option>
                </select>
              </div>
              <div><?php $this->field( $this->tt('Valor','Value'), 'commission_value', 'number', '15', true, 'min="0" step="0.01"' ); ?></div>
              <div><?php $this->field( $this->tt('Notas internas','Internal notes'), 'notes', 'text' ); ?></div>
            </div>
            <div style="display:flex;gap:10px;">
              <button type="submit" class="button button-primary"><?php echo esc_html( $this->tt( 'Crear partner y generar QR', 'Create partner and generate QR' ) ); ?></button>
              <button type="button" onclick="document.getElementById('amir-new-partner-form').style.display='none';" class="button"><?php echo esc_html( $this->tt( 'Cancelar', 'Cancel' ) ); ?></button>
            </div>
          </form>
        </div>
        ...
        <table style="width:100%;border-collapse:collapse;">
          <thead><tr style="background:#f8fdfb;">
            ...
            <th class="ab-th"><?php echo esc_html( $this->tt( 'Ventas', 'Sales' ) ); ?></th>
            <th class="ab-th"><?php echo esc_html( $this->tt( 'Comisión ganada', 'Commission earned' ) ); ?></th>
          ...
          <?php foreach ($partners as $p) :
            ...
            <tr style="border-bottom:1px solid #f5f5f5;">
              ...
              <td class="ab-td">
                ...
                <?php else : ?>
                  <strong>$<?php echo number_format($p->commission_value,2); ?></strong> <?php echo esc_html( $this->tt( 'fijo/reserva', 'flat/booking' ) ); ?>
                <?php endif; ?>
              </td>
              <td class="ab-td">
    ...
    private function render_detail( int $partner_id, string $message = '' ): void {
        ...
        if (!$p) { echo '<div class="wrap"><p>' . esc_html( $this->tt( 'Partner no encontrado.', 'Partner not found.' ) ) . '</p></div>'; return; }
        ...
        <div style="display:grid;grid-template-columns:1fr 320px;gap:20px;">
          <div>
            ...
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:20px;">
              <?php foreach ([
                [ $this->tt('Total ventas','Total sales'), \AmirBooking\Core\Currency::format( (float) $stats['total_revenue'], 0 ) ],
            ...
            <!-- Últimas reservas -->
            <div style="font-size:14px;font-weight:700;color:#1a2e24;margin-bottom:10px;"><?php echo esc_html( $this->tt( 'Últimas reservas referidas', 'Latest referred bookings' ) ); ?></div>
            <div style="background:#fff;border:1px solid #e1f5ee;border-radius:10px;overflow:hidden;">
            <table style="width:100%;border-collapse:collapse;">
              <thead><tr style="background:#f8fdfb;">
                <th class="ab-th"><?php echo esc_html( $this->tt( 'Referencia', 'Reference' ) ); ?></th><th class="ab-th">Tour</th><th class="ab-th"><?php echo esc_html( $this->tt( 'Fecha', 'Date' ) ); ?></th><th class="ab-th"><?php echo esc_html( $this->tt( 'Total', 'Total' ) ); ?></th><th class="ab-th"><?php echo esc_html( $this->tt( 'Comisión', 'Commission' ) ); ?></th>
              </tr></thead>
              <tbody>
          ...
          <div>
            <div style="background:#fff;border:1px solid #e1f5ee;border-radius:10px;padding:16px;">
              ...
              <div style="margin-bottom:16px;">
                ...
                <?php if (!empty($urls['qr_all_tours'])) : ?>
                  <div style="text-align:center;margin-bottom:8px;">
                    <img src="<?php echo esc_url($urls['qr_all_tours']); ?>" alt="<?php echo esc_attr( $this->tt( 'QR Catálogo', 'QR Catalog' ) ); ?>" style="width:120px;height:120px;border:1px solid #e1f5ee;padding:6px;border-radius:6px;" />
                    <div style="font-size:11px;color:#5a7068;margin-top:4px;"><?php echo esc_html( $this->tt( 'QR → Catálogo de tours', 'QR → Tour catalog' ) ); ?></div>
                  </div>
                  <a href="<?php echo esc_url($urls['qr_all_tours']); ?>" download
                     class="button" style="width:100%;text-align:center;box-sizing:border-box;display:block;">⬇ <?php echo esc_html( $this->tt( 'Descargar QR (catálogo)', 'Download QR (catalog)' ) ); ?></a>
                <?php endif; ?>
              </div>
              ...
              <?php if (!empty($urls['tours'])) :
                foreach ($urls['tours'] as $t) : ?>
                  <div style="border-top:1px solid #e1f5ee;padding-top:12px;margin-top:12px;">
                    ...
                    <?php if ($t['qr']) : ?>
                      <a href="<?php echo esc_url($t['qr']); ?>" download class="button" style="font-size:12px;">⬇ <?php echo esc_html( $this->tt( 'QR tour', 'Tour QR' ) ); ?></a>
                    <?php endif; ?>
                  </div>
                <?php endforeach;
              endif; ?>
              ...
              <form method="post" style="margin-top:14px;border-top:1px solid #e1f5ee;padding-top:14px;">
                ...
                <button type="submit" class="button" style="width:100%;">🔄 <?php echo esc_html( $this->tt( 'Regenerar QR', 'Regenerate QR' ) ); ?></button>
              </form>
            </div>
